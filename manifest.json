...
// Game progress heartbeat - typically longer intervals for gameplay tracking
const GAME_HEARTBEAT_INTERVAL_MS = 15000;
...
function isGameQuest(quest: any): boolean {
    return !!quest?.config?.taskConfigV2?.tasks?.PLAY_ON_DESKTOP || !!quest?.config?.taskConfigV2?.tasks?.PLAY_ON_MOBILE;
}
...
// Sends game-progress heartbeats until completed_at flips non-null.
async function playGame(rest: ReturnType<typeof getRestClient>, quest: any): Promise<boolean> {
    const taskType = quest.config.taskConfigV2.tasks.PLAY_ON_DESKTOP ? "PLAY_ON_DESKTOP" : "PLAY_ON_MOBILE";
    const target: number = quest.config.taskConfigV2.tasks[taskType].target;
    let elapsed = 0;

    while (elapsed < target) {
        await sleep(GAME_HEARTBEAT_INTERVAL_MS);
        elapsed = Math.min(elapsed + GAME_HEARTBEAT_INTERVAL_MS / 1000, target);

        const res = await rest!.post({
            url: `/quests/${quest.id}/game-progress`,
            body: { timestamp: elapsed },
        });
...
export async function completeAllGameQuests(): Promise<QuestRunResult> {
    const result: QuestRunResult = { completed: [], needsManualClaim: [], failed: [], skipped: 0 };
    ...
    const quests: any[] = Array.from(questStore.quests.values()).filter(isGameQuest);
    ...
    for (const quest of quests) {
        try {
            ...
            if (!quest.userStatus?.completedAt) {
                const done = await playGame(rest, quest);
                if (!done) {
                    result.failed.push(questName(quest));
                    continue;
                }
...
export async function completeAllQuests(): Promise<{ video: QuestRunResult; game: QuestRunResult }> {
    const videoResult = await completeAllVideoQuests();
    const gameResult = await completeAllGameQuests();
    return { video: videoResult, game: gameResult };
}
